...
document.querySelectorAll('.checkbox').forEach((element) => {
  element.nextElementSibling.addEventListener('click', (e) => {
    e.target.parentElement.setAttribute('style', 'background-color:#eeeeee;');
    e.target.nextElementSibling.classList.remove('fa-trash-can', 'fa-xs', 'fa-ellipsis-vertical');
    e.target.nextElementSibling.classList.add('fa-trash-can', 'fa-xs', 'delete');
    ...
    document.querySelectorAll('.checkbox').forEach((element) => {
      if (element.nextElementSibling !== e.currentTarget) {
        ...
        element.nextElementSibling.nextElementSibling.classList.add('fa-trash-can', 'fa-xs', 'fa-ellipsis-vertical');
        element.nextElementSibling.nextElementSibling.classList.remove('fa-trash-can', 'fa-xs', 'delete');
      }
    });
  });
});
